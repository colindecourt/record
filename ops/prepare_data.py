...
from ops.confidence_map import generate_confmap, normalize_confmap, add_noise_channel
from utils.args import update_config_dict
...
def generate_confmaps(metadata_dict, n_class):
    confmaps = []
...
def prepare_data(dataset, config_dict, data_dir, split, save_dir, overwrite=False):
    """
    ...
    for seq in sets_seqs:
        ...
        try:
            ...
            if split == 'demo' or not os.path.exists(seq_anno_path):
                ...
                anno_obj['confmaps'] = generate_confmaps(anno_obj['metadata'], n_class)
...
if __name__ == "__main__":
    ...
    config_dict = yaml.load(open(args.config), yaml.FullLoader)
    config_dict = update_config_dict(config_dict, args)  # update configs by args
    radar_configs = dataset.sensor_cfg.radar_cfg

    if splits == None:
        prepare_data(dataset, config_dict, out_data_dir, split=None, save_dir=out_data_dir,
                     overwrite=overwrite)
    else:
        for split in splits:
        ...
        for split in splits:
            ...
            prepare_data(dataset, config_dict, out_data_dir, split, save_dir, overwrite=overwrite)